...
import {
  PaymentFormValues,
  paymentSchema,
} from "@/modules/classroomPage/CreatePaymentFormModal/CreatePaymentFormModal.helpers";
...
export const UpdatePaymentFormModal = ({
  payment,
  studentId,
  open,
  onOpenChange,
  ...
  const { mutate: updatePayment, isPending } = useUpdatePayment(studentId);
  ...
  useEffect(() => {
    if (payment) {
      ...
      setValue("forMonth", paymentDate);
      setSelectedMonth(paymentDate.getMonth());
      setSelectedYear(paymentDate.getFullYear());
    } else {
      reset();
